...
\frame{
  ...
  Before we start analysing the data, it is important to learn an important computational research skill \ldots
}
...
\frame{
  ...
  Reproducible reasearch is when an {\emph{athe data and code used to make a finding are available and they are sufficient for an independent researcher to make the same findings (see Peng 2011).}} \\[0.5cm]
}
...
\frame{
  As you will see, making your research reproducible requires extra an {\bf{upfront}} investment. \\[0.5cm]
...
\frame{
  ...
  Second, then we will learn how to use the {\emph{knitr}} package to weave our {\emph{source code}} and {\emph{Markdown presentation documents}}.
}
...
\frame{
  ...
  \begin{itemize}
    Source Code $\Longleftrightarrow$ knitr package $\Longleftrightarrow$ Markdown  
  \end{itemize}
}
