...
\frame{
  ...
  {\Large{To answer this question we will need tools of {\bf{statistical inference}}. \\[1cm]
...
\frame{
  ...
  Especially, since when we take {\bf{other samples}} we will get {\bf{different estimates}}.}}
  \end{center}
}
...
\frame{
  \frametitle{Question}
  \begin{center}
    {\Large{What would the sampling distribution of the sample mean look like if the sample size was 5,000 rather than 100?}}
  \end{center}
}

\begin{frame}[fragile]
    \frametitle{Comparing Sampling Distributions of $\bar{x}_{test}$ 100 vs. 5,000}
...
ggplot(SampleDistMolten, aes(x = value, colour = factor(SampleSize), fill = factor(SampleSize))) + 
        ...
        geom_vline(xintercept = OneTimeMean, colour = "#8DA0CB", linetype = "longdash") +
        geom_vline(xintercept = PopMeanTime, colour = "#1F78B4", linetype = 1) +
        annotate("text", x = 99, y = 1.5, label = "Our 1st\n Sample Mean", colour = "#8DA0CB") +
        annotate("text", x = 99, y = 2, label = "Population\n Mean", colour = "#1F78B4") +
        xlab("\nSample Means of Time") + ylab("Density\n") + 
...
\frame{
  ...
  \begin{center}
    {\Large{The {\bf{likely distance}} between the sample mean and the population mean is a related to {\bf{sample size}} ($n$), \\[1cm]
...
\frame{
  \frametitle{Finally, the Standar Error!}
...
\frame{
  ...
  {\Large{The standard error is a measure of our {\bf{uncertainty}} about how well the point estimate represents the population parameter.}}
}
...
@
  ...
  {\large{So, \texttt{time} in our original sample with 100 observations had a sampleing mean of \Sexpr{round(mean(Run10Samp$time), digit = 1)} and a standard error of \Sexpr{round(std.error(Run10Samp$time), digit = 1)}.
\end{frame}
...
\frame{
  \frametitle{Um\ldots}
  {\Large{Um \ldots a sampleing mean of \Sexpr{round(mean(Run10Samp$time), digit = 1)} and a standard error of \Sexpr{round(std.error(Run10Samp$time), digit = 1)}? \\[1cm]
  That's not very easy to understand.}}
}

\frame{
  \frametitle{We need something more usefull.}
...
\begin{frame}[fragile]
  ...
  {\Large{About {\bf{95\%}} of the time the population paramenter will be within {\bf{about 2 standard errors}} of the point estimate. \\[1cm]
  95\% Confidence Interval ($CI$):}}
  \[
    CI = \mathrm{point\:estmiate} \pm 2 * SE
  \]
\end{frame}
...
\frame{
  ...
  \[
    CI = \mathrm{point\:estmiate} \pm 1.96 * SE
  \]
}
