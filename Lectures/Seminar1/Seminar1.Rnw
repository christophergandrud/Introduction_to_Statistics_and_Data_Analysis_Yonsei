...
\begin{frame}[fragile]
  \frametitle{Object Classes}
      {\LARGE{Object Classes}} \\[0.5cm]
      All objects have a {\bf{class}}. \\[0.5cm]
      Some are {\bf{numeric}}, some are {\bf{characters}}, some are {\bf{logical}} (TRUE or FALSE) \\[0.25cm]
...
\frame{
  ...
  \begin{itemize}
    \item<1-> The main type of object we will use in this class is called a {\bf{dataframe}}.
        \begin{itemize}
          \item<1-> We will cover dataframes in detail next class. \\[0.5cm]
        \end{itemize}
...
\frame{
  \frametitle{Vectors}
  {\LARGE{Vectors}} \\[0.5cm]
  Vectors are objects with multiple numbers {\emph{or}} character strings in a particular order.
}
