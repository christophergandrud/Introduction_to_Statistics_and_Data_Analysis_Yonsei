...
\frame{
  ...
  But how do we usually {\bf{get data into R}} and make it useable for data analysis?}}
}
...
\frame{
  ...
  \begin{itemize}
    ...
    \item<3-> What do Southern European countries have higher deficits than Northern European ones?
...
\frame{
  ...
  \begin{itemize}
    \item<2-> The rich people I know vote for Party A not Party B.
  \end{itemize} \\[0.25cm]
  ...
  Anecdotal evidence is not representative of the population of interest and {\bf{should not}} be used to make {\bf{generalizable}} conclusions.
}
...
\frame{
  ...
  {\LARGE{Sampling in this Course}} \\[0.25cm]
  When we use convenience samples we {\bf{must always look out for factors that might bias our results}}. \\[0.5cm]
...
\frame{
  ...
  The response variable is also sometimes called the {\bf{dependent variable}} or denoted by $Y$.
...
\frame{
  ...
  The explanatory variable is also sometimes called the {\bf{independent variable}} or denoted by $X$.
...
\frame{
  ...
  \begin{itemize}
    \item Are simple,
    \item Can easily be loaded into R.
  \end{itemize}
...
\frame{
  ...
  You can find {\bf{other data API packages}} at: \url{http://bit.ly/Qw16RY}. \\[0.5cm]
  The {\bf{MacroData Guide}} is a good source of social science data.
  \begin{itemize}
    \item \url{http://www.nsd.uib.no/macrodataguide/topic.html}
  \end{itemize} \\[0.5cm]
  The Federal Reserve Bank of St. Louis {\bf{FRED}} database.
  \begin{itemize}
    \item \url{http://research.stlouisfed.org/fred2/}
  \end{itemize} \\[0.5cm]
...
\begin{frame}[fragile]
  ...
  To save your data to a plain-text file use \texttt{write.table}.
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
write.table\{LifeExpect, 
            file = \hlstring{"LifeExpectacyData.csv"}, sep = \hlstring{","}\}
\end{alltt}
\end{kframe}
\end{knitrout}
