...
\frame{
  ...
  \begin{table}
    ...
    \end{tabular}
  \end{table}
  Find the {\bf{sample proportions}} for each racial group. \\[0.5cm]
...
\frame{
  \frametitle{$\chi^{2}$ (1)}
  Our hypotheses were about how whether {\bf{all}} of the sample proportions were different from the population proportions. \\[0.5cm]
...
\frame{
  ...
  We can't assume that the $\chi_{2}$ statistic follows a normal or $t$ distribution if the null hypothesis is true. \\[0.5cm]
  Instead, we use a {\bf{$\chi^{2}$ distribution}}. \\[0.5cm]
  It's only parameter is the {\bf{degrees of freedom}} ($df$). \\[0.25cm]
...
\frame{
  \frametitle{Hypothesis Testing}
{\LARGE{Null Hypothesis:}} \\[0.5cm]
  $H_{0}:$ There is no difference in university enrollement at age 21 between disadvantaged children who attended preschool and those who didn't. (dependent) \\[0.5cm]
  
{\LARGE{Alternative Hypothesis:}} \\[0.5cm]
  $H_{A}:$ There is a difference in university enrollement at age 21 between disadvantaged children who attended preschool and those who didn't. (independent)
}
...
\frame{
  ...
  The general formula for finding {\bf{expected count}} for a row $i$ in column $j$ is:
  \[
...
\frame{
  ...
  \begin{itemize}
    \item<1->determine likely population parameters from a sample, especially the mean \& proportions, 
    \item<1-> determine if groups are independent.
  \end{itemize}
...
\frame{
  \frametitle{Intreting $\beta$}
{\LARGE{Interpretting $\beta$:}} \\[0.5cm]
...
\frame{
  \frametitle{Infact\ldots}
  Infact, if you find a perfectly linear or almost perfectly linear relationship in social science research, you probably have a {\bf{problem}}. \\[0.5cm]
