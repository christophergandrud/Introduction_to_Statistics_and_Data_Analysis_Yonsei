...
{\Large{Remember that the coefficients are in terms of the variables' units. \\[0.5cm]
  A large coefficient does not neccessarily mean a big effect.}}
  \end{center}
}
...
\begin{frame}[fragile]
  ...
  Luckily, R will quicly do this for us with the \texttt{summary} command:
{\tiny{
<<RoughSummary>>=
summary(M1)
@
...
<<ConfIntGraph, echo=FALSE, message=FALSE, fig.height=4>>=
library(reshape)
library(ggplot2)
...
\frame{
  ...
  We don't want to just include every varibale we can think of into one model.
  \begin{itemize}
...
\frame{
  \frametitle{Model Selction as Art}
  ...
  \begin{itemize}
    ...
    \item \textbf{Show a number of step-wise models} that could convince the reader that you have found the most {\bf{parsimonous}} model.
...
\frame{
  ...
  They (and others) created the \texttt{Zelig} package to help us simulate and graph expexted outcomes.
}
...
\begin{frame}[fragile]
<<Zelig3, tidy=FALSE, fig.height=4>>=
