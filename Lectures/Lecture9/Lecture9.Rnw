...
{\large{They are the $\mathrm{Z}$ score for the confidence level. \\[0.5cm]
  $\mathrm{Z}$ scores are number of standard deviations above or below the mean that an observation falls\\[0.5cm]
  ...
  \begin{itemize}
    ...
    \item<1-> The distribution is not extremely skewed.
  \end{itemize}
...
{\Large{What if the Central Limit Theorem's Assumptions Aren't Met?}} \\[0.5cm]
  \begin{itemize}
    ...
    \item If you have a small sample (about $n < 50$) that is not highly skewed, you can use the $t$ distribution to calculate confidence intervals. See Diaz et al. (2011, Ch. 6).
  \end{itemize}
}
...
{\LARGE{p-value}} \\[0.5cm]
  ...
  In our previous {\bf{example}}, we would want to know the probability of seeing a mean of at least \Sexpr{round(mean(Run10$time), digit = 1)} if the true population mean is actually 93.29 (the population mean for 2006).
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Example}
{\LARGE{Example}} \\[0.5cm]
  In our previous {\bf{example}}, we would want to know the probability of seeing a mean of at least \Sexpr{round(mean(Run10$time), digit = 1)} if the true population mean is actually 93.29 (the population mean for 2006). \\[0.5cm]
\end{frame}

\begin{frame}[fragile]
\
  We first calculated the $\mathrm{Z}$ score of the sample mean $\bar{x} = \Sexpr{round(mean(Run10$time), digit = 1)}$
\[
...
\begin{frame}[fragile]
  \frametitle{Find p-values}
  To find the p-value for the \Sexpr{round(Z, digit = 1)} use \texttt{pnorm}.
<<pnorm1tail>>=
P1 <- 1 - pnorm(Z)
P1
@
...
This is bigger than the 0.05 significance level. So, we fail to reject the null hypothesis that $\mu{x}_{09} > \mu{x}_{06}$.
\end{frame}

\begin{frame}[fragile]
  \frametitle{Um\ldots}
  But we were interested in the null hypothesis $\mu{x}_{09} = \mu{x}_{06}$, not $\mu{x}_{09} > \mu{x}_{06}$. \\[0.5cm]
...
P2
@
We fail to reject the null hypothesis that $\mu{x}_{09} = \mu{x}_{06}$. \\[0.25cm]
